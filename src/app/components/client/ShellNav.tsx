...
export default function ShellNav() {
    ...
    const { loading, error, lastUpdated, polling, markets, demo } = useBoardContext();
    const network = strk20.networkName ?? "network";
    const live = !loading && !error && markets.length > 0 && polling;
    const status = demo ? `${network} · demo` : live ? network : error ? `${network} · unreadable` : network;
        ...
        <header className={styles.glassNav}>
            <nav className={uni.nav}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <span className={`${styles.liveDot} ${demo || !live ? styles.liveDotMuted : ""}`} title={demo ? "Showing seeded demo data" : lastUpdated ? `Last read ${formatAge(lastUpdated)}` : "Waiting for first read"}>
                        {demo ? "DEMO" : "LIVE"}
                    </span>
                </div>
