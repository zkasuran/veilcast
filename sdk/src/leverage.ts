...
import { LEVERAGED_MARKET_ABI } from "./abi.js";
...
/// Domain separator for the close signature, matching `CLOSE_MESSAGE_TAG` in the contract.
...
export type LevMarketView = {
    ...
    closeAt: number;
    createdAt: number;
    /// FPMM reserves. Marginal YES price is rNo / (rYes + rNo).
    rYes: bigint;
    rNo: bigint;
...
/// A leveraged position's bearer proof, minted per open and kept off-chain by the trader. The
/// private key is what closes the position and collects the equity; the public key is the only
/// handle the contract sees, so two positions by one person share nothing on-chain. Lose it and the
/// margin is stranded; hold a copy and you can close.
export type LevCoupon = {
    marketId: number;
    side: number;
    privateKey: string;
    positionKey: string;
    /// Trader collateral, the smallest unit, as a decimal string. The most the trader can lose.
    margin: string;
    leverageBps: number;
    createdAt: number;
};
...
/// `[1, market_id, side, position_key, r, s, 0, note_id]`. Signing a zero target is a bearer
...
/// Floor integer square root, the bigint mirror of Cairo's `u256` sqrt. Used only to quote a sell
/// the same way the contract computes it.
...
/// Buy `amount` collateral worth of the `bought` outcome. Returns the shares out and the new
/// reserves, rounding so the pool is never left short. Mirrors `pricing::buy`.
export function buy(
    rBought: bigint,
    rOther: bigint,
    amount: bigint
...
/// Sell `shares` of the `sold` outcome back to the pool. Returns the collateral out and the new
/// reserves, rounded down. Mirrors `pricing::sell`.
...
export async function loadVaultShares(
    provider: ProviderInterface,
    address: string,
    lp: string
): Promise<bigint> {
...
// ── Public calls (account.execute): liquidity and the market's public admin ──

/// Provide liquidity. The LP must approve `amount` of STRK to the contract first, so submit this
/// alongside the approval in one multicall.
...
/// Burn vault shares for a pro-rata slice of the free vault collateral.
...
/// Open a leveraged market seeded from the vault at an even 50/50.
...
/// Settle a closed market on `winningSide`. Resolver only, enforced on-chain.
...
/// Cancel a market so every position reclaims its margin. Resolver only.
...
/// the contract to book the position. One atomic transaction, sender recorded as the pool.
...
/// The pool transaction that closes a position to a private note: open a note, then invoke the
/// contract to fill it with the equity. Only for a position that still has equity to pay out.
